...
def test_platform_scorecards():
    metrics = scorecards.platform_scorecards(
        _ngos(),
        [{"lives_impacted": 100}, {"lives_impacted": 50}],
        [{"status": "verified"}, {"status": "open"}, {"status": "resolved"}],
    )
    assert metrics["ngos_registered"] == 3
    assert metrics["tasks_completed"] == 18
    assert metrics["lives_impacted"] == 150
    assert metrics["needs_resolved"] == 2
